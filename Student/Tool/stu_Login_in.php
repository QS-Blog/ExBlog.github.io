...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // SQL 查询语句
    $sql = "SELECT * FROM Students WHERE id = $studentId AND password = '$password'";
    echo $sql;
    ...
    if ($result->num_rows > 0) {
        ...
        $_SESSION["signDate"] = $user["signDate"]; // 假设还有一个签到日期字段
        $_SESSION['isSign'] = $user['isSign']; // 假设还有一个是否签到字段
        $_SESSION['avatar'] = $user['avatar']; // 假设还有一个头像字段
        $_SESSION['type'] = "student";
        echo "登录成功！";
        // 可以在这里重定向到主页或其他页面
        header("Location: ..\Php\studentPage.php"); // 为实际主页链接
        exit();
